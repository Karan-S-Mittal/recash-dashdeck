...
page_container = html.Div(
    [dcc.Location(id=_ID_LOCATION), html.Div(
        id=_ID_CONTENT), html.Div(id=_ID_DUMMY)]
)
...
def register_page(
    ...
    ***
    ...
    page.update(
        ...
        description=(
            description if description is not None else page["title"]),
...
def _infer_image(module):
    ...
    files_in_assets = [f for f in listdir(
        "assets") if isfile(join("assets", f))]
    app_file = None
    logo_file = None
    ...
    def router():
        ...
        for module in dash.page_registry:
            ...
            app.validation_layout = html.Div(
                [
                    page["layout"]() if callable(
                        page["layout"]) else page["layout"]
            ...
            if page["redirect_from"] and len(page["redirect_from"]):
                for redirect in page["redirect_from"]:
                    ...
                    app.server.add_url_rule(
                        redirect, redirect, create_redirect_function(
                            page["path"])
                    )
